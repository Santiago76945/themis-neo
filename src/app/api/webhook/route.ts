...
function verifySignature(signatureHeader: string | null, requestId: string | null, resourceId: string | null): boolean {
    if (!WEBHOOK_SECRET || !signatureHeader || !requestId || !resourceId) {
        console.error("Webhook verification failed: missing data", { WEBHOOK_SECRET: !!WEBHOOK_SECRET, signatureHeader, requestId, resourceId });
        return false;
    }
    // signatureHeader example: "ts=1680000000000,v1=abcdef123456..."
...
export async function POST(req: NextRequest) {
    ...
    if (topic === "payment" && id) {
        try {
            ...
            const mpRes: any = await (paymentClient as any).get(id);
